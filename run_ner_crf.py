...
def train(args,train_dataset,model,tokenizer):
    ...
    train_dataloder = DataLoader(train_dataset,sampler=train_sampler,batch_size=args.train_batch_size,collate_fn=collate_fn)

    if args.max_steps > 0:
        t_total = args.max_steps 
        args.num_train_epochs = args.max_steps // len(train_dataset) // args.gradient_accumulation_steps +1
    else:
        t_total = len(train_dataset) * args.num_train_epochs // args.gradient_accumulation_steps
    ...
    optimizer_group_parameters = [
        ...
        {
            "params":[p for n,p in bert_model_parameters if any([nd in n for nd in no_decay])],
        ...
        {
            "params":[p for n,p in crf_model_parameters if any([nd in n for nd in no_decay])],
            "weight_decay":0.0,"lr":args.crf_learning_rate
        },
        {
            "params":[p for n,p in linear_model_parameters if not any([nd in n for nd in no_decay])],
            "weight_decay":args.weight_decay,"lr":args.crf_learning_rate
        },
        {
            "params":[p for n,p in linear_model_parameters if any([nd in n for nd in no_decay])],
            "weight_decay":0.0,"lr":args.crf_learning_rate
        }
    ...
    if os.path.exists(args.model_name_or_path) and "checkpoint"  in args.model_name_or_path:
        ...
        steps_trained_in_current_epoch = global_step % ((len(train_dataloder)) // args.gradient_accumulation_steps)
        logger.info("Continuing training from checkpoit, will skip to saved global_step")
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        for step,batch in enumerate(train_dataloder):
            ...
            if args.n_gpu > 1:
                loss = loss.mean() # mean to average on multi-gpu parallel trianing
...
def load_and_cache_examples(args,task_name,tokenizer,data_type="train"):
    ...
    cached_features_file = os.path.join(args.data_dir,"cache_crf-{}_{}_{}_{}".format(data_type,
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        logger.info("Creating features from dateset file at %s",cached_features_file)
...
def main():
    ...
    if os.path.exists(args.output_dir)  and os.listdir(args.output_dir) and args.do_train and not args.overwrite_output_dir:
        raise ValueError(
            "Output directory ({}) is already exists and is not empty.use --overwrite_output_dir to overcome.".format(args.output_dir)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0 ):
        ...
        model_to_save = (model.module if model is hasattr(model,"module") else model)
    ...
    if args.do_eval and args.local_rank in [-1,0]:
        ...
        if args.eval_all_checkpoints:
            ...
            logging.getLogger("pytorch_transformers.modeling_utils").setLevel(logging.WARNING)
